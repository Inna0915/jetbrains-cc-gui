...
- Agy image attachments are passed to `Agent.chat` as SDK `Image` primitives in API-key mode; local CLI auth mode rejects image requests with an actionable configuration error.
